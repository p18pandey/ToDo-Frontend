...
const TaskList = () => {
    ...
    const fetchTasks = async () => {
        try {
            const response = await axios.get('https://todo-backend-0la0.onrender.com/tasks');
            setTasks(response.data);
        } catch (error) {
            console.error('Error fetching tasks:', error);
        }
    ...
    const handleEdit = async (id, updatedTask) => {
        try {
            const response = await axios.put(`https://todo-backend-0la0.onrender.com/tasks/${id}`, updatedTask);
            setTasks((prev) =>
                prev.map((task) => (task._id === id ? response.data : task))
            );
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`https://todo-backend-0la0.onrender.com/tasks/${id}`);
            setTasks((prev) => prev.filter((task) => task._id !== id));
        } catch (error) {
            console.error('Error deleting task:', error);
        }
    ...
    const handleStatusChange = async (taskId, newStatus) => {
        try {
            const response = await axios.patch(
                `https://todo-backend-0la0.onrender.com/tasks/${taskId}/status`,
                { status: newStatus }
            );
            setTasks((prevTasks) =>
                prevTasks.map((task) =>
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 mt-4 sm:mt-0">
                                    <div className="relative group">
                                        <button
                                            ...
                                            className={`px-4 py-2  text-sm font-medium rounded-md shadow-md transition duration-200 focus:outline-none focus:ring-2 ${
                                                task.status === "pending"
                                                    ? "text-yellow-700 bg-yellow-100 hover:bg-yellow-200 focus:ring-yellow-400"
                                                    : "text-green-700 bg-green-100 hover:bg-green-200 focus:ring-green-400"
                                            }`}
